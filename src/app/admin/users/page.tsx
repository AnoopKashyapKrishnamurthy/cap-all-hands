...
import { AdminLoginAccessControl, AdminRoleControl } from '@/components/admin/AdminActions'
...
export default async function AdminUsersPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  let query = supabase.from('user_profiles').select('id, email, display_name, avatar_url, role, login_enabled, created_at', { count: 'exact' })
  ...
  return (
    <AdminPage>
      <AdminPageHeader title="Users" description="Manage application login access and authorization roles." />
      ...
      {error ? <AdminErrorState /> : !data?.length ? <AdminEmptyState title="No users found" description="Try a different name, email, or role filter." /> : (
        <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white shadow-sm">
          <table className="w-full min-w-[860px] text-left text-sm">
            <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-5 py-3">Person</th><th className="px-5 py-3">Role</th><th className="px-5 py-3">Login access</th><th className="px-5 py-3">Joined</th><th className="px-5 py-3">Change role</th></tr></thead>
            <tbody className="divide-y divide-slate-100">
              {data.map((profile) => (
                <tr key={profile.id} className="align-top">
                  ...
                  <td className="px-5 py-4"><AdminLoginAccessControl userId={profile.id} enabled={profile.login_enabled} isCurrentAdmin={profile.id === user.id} /></td>
